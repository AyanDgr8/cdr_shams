...
/**
 * Update follow-up notes in final_report table from raw_cdrs_all
 * @returns {Promise<Object>} - Summary of the operation
 */
async function updateFollowUpNotes() {
  try {
    console.log('🔄 Starting follow-up notes update process...');
    ...
    if (finalReportRecords.length === 0) {
      ...
      return { updatedCount: 0, notFoundCount: 0, noNotesCount: 0 };
    }
    ...
    return { updatedCount, notFoundCount, noNotesCount };
    
  } catch (error) {
    console.error('❌ Error during follow-up notes update:', error);
    return { updatedCount: 0, notFoundCount: 0, noNotesCount: 0, error: error.message };
  }
}
...
async function populateFinalReportEnhanced(tenant, params = {}) {
  ...
  try {
    ...
    // Step 4: Update follow-up notes in final_report table
    console.log('\n📊 Step 4: Updating follow-up notes in final_report table...');
    const followUpNotesResult = await updateFollowUpNotes();
    ...
    return {
      ...
      followUpNotesUpdated: followUpNotesResult.updatedCount
    };
  } catch (error) {
...
async function testEnhancedFinalReport() {
  try {
    ...
    const populateResult = await populateFinalReportEnhanced('shams', {
